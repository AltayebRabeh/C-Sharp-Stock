...
namespace Stock.PL.ReRequests
{
    public partial class Add : Form
    {
        ...
        {
            Choose frm = new Choose("requests_reback");
            frm.ShowDialog();
            if (frm.dgv.Rows.Count > 0)
            {
                txt_re_id.Text = frm.dgv.CurrentRow.Cells[0].Value.ToString();
                txt_re_date.Text = frm.dgv.CurrentRow.Cells[1].Value.ToString();
                su_name.Text = frm.dgv.CurrentRow.Cells[2].Value.ToString();
                txt_driver_name.Text = frm.dgv.CurrentRow.Cells[3].Value.ToString();
                txt_driver_name.Enabled = txt_re_date.Enabled = su_name.Enabled = false;
            }
        ...
        private void btn_add_product_Click(object sender, EventArgs e)
        {
            ...
            if (txt_re_id.Text == string.Empty)
            {
                MessageBox.Show("الرجاء اختيار الفاتورة");
                return;
            }

            Choose frm = new Choose("requests_details", txt_re_id.Text);
            frm.ShowDialog();
        ...
        {
            txt_driver_name.Text = txt_re_date.Text = txt_re_id.Text = su_name.Text = string.Empty;
            txt_driver_name.Enabled = txt_re_date.Enabled = su_name.Enabled = true;
        }
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgv.Rows.Count; i++)
            {
                decimal qty = Convert.ToDecimal(BL.Products.qty(Convert.ToInt32(dgv.Rows[i].Cells[0].Value.ToString())));
                if (dgv.Rows[i].Cells[3].ToString() != "")
                {
                    if (qty < Convert.ToDecimal(dgv.Rows[i].Cells[2].Value.ToString()) * Convert.ToDecimal(dgv.Rows[i].Cells[4].Value.ToString())
                        + Convert.ToDecimal(dgv.Rows[i].Cells[3].Value.ToString()))
                    {
                ...
                {
                    if (qty < Convert.ToDecimal(dgv.Rows[i].Cells[2].Value.ToString()))
                    {
                        MessageBox.Show("الكميات في المخزن اقل من الكميات المراد حذفها");
                        return;
                    }
            ...
            re_id = Convert.ToInt32(txt_re_id.Text);
            ...
            BL.ReRequests.Save(d_id, Program.UserID, re_id);
            re_re_id = Convert.ToInt32(BL.ReRequests.GetLastReRequestId());

            for (int i = 0; i < dgv.Rows.Count; i++)
            {
                if (int.Parse(dgv.Rows[i].Cells[4].Value.ToString()) == 0)
                {
                    BL.ReRequests.SaveDetails(Convert.ToInt32(dgv.Rows[i].Cells[0].Value.ToString()), Convert.ToDecimal(dgv.Rows[i].Cells[2].Value.ToString()), 0, re_re_id);
                ...
                {
                    BL.ReRequests.SaveDetails(Convert.ToInt32(dgv.Rows[i].Cells[0].Value.ToString()), Convert.ToDecimal(dgv.Rows[i].Cells[2].Value.ToString()), Convert.ToDecimal(dgv.Rows[i].Cells[3].Value.ToString()), re_re_id);
        ...
        private void btn_new_Click(object sender, EventArgs e)
        {
            ...
            txt_p_id.Text = txt_p_name.Text = txt_qty.Text = txt_min_qty.Text =
                txt_re_id.Text = txt_re_date.Text = su_name.Text = txt_driver_name.Text =
                txt_d_id.Text = txt_d_name.Text = txt_d_natnum.Text = txt_car_type.Text = txt_car_num.Text = string.Empty;
            lbl_min_qty.Text = "الكمية بــ ";
            lbl_qty.Text = "الكمية بــ ";
            txt_re_id.Enabled = txt_re_date.Enabled = su_name.Enabled = txt_driver_name.Enabled =
        ...
        private void btn_print_Click(object sender, EventArgs e)
        {
            ...
            rep.Parameters["parameter1"].Value = re_re_id;
            frm.ShowDialog();
        }
    }
}

...
namespace Stock.PL.ReRequests
{
    public partial class All : Form
    {
        ...
        private void btn_del_Click(object sender, EventArgs e)
        {
            if (dgv.Rows.Count > 0)
            {
                ...
                if(dr == DialogResult.Yes) {
                    dt = BL.ReRequests.ReRequestsDetails(int.Parse(dgv.CurrentRow.Cells[0].Value.ToString()));
                    ...
                    int count = BL.ReRequests.Delete(Convert.ToInt32(dgv.CurrentRow.Cells[0].Value.ToString()));
                    All_Activated(null, null);
        ...
        private void txt_search_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                dgv.DataSource = BL.ReRequests.Search(txt_search.Text);
            }
            else
            {
                dgv.DataSource = BL.ReRequests.Search(txt_search.Text, 1);
            }
        }
        ...
        private void cb_inv_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cb_inv.Text == string.Empty)
            {
                dgv.DataSource = BL.ReRequests.AllReRequests();
                btn_del.Enabled = true;
            }
            else
            {
                dgv.DataSource = BL.ReRequests.AllReRequests(1);
